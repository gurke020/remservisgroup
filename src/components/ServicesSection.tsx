  ...
  return (
    <section id="services" className="section-padding">
      <div className="container">
        ...
        <div ref={gridRef} style={{ opacity: 0 }} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-5">
          {services.map((s, i) => (
            <div
              key={s.title}
              className="group relative bg-card rounded-xl p-6 hover-lift border border-border/40 overflow-hidden"
              style={{ animationDelay: `${i * 80}ms` }}
            >
              {/* Hover accent bar */}
              <div className="absolute top-0 left-0 right-0 h-[3px] bg-gradient-to-r from-primary via-accent to-cta opacity-0 group-hover:opacity-100 transition-opacity duration-300" />

              <div className="w-11 h-11 rounded-lg bg-primary/8 flex items-center justify-center mb-4 group-hover:bg-cta/10 transition-colors duration-300">
                <s.icon className="w-5 h-5 text-primary group-hover:text-cta transition-colors duration-300" strokeWidth={1.8} />
              </div>

              <h3 className="text-lg font-semibold mb-1.5">{s.title}</h3>
              <p className="text-muted-foreground text-sm leading-relaxed mb-4">{s.desc}</p>

              <p className="text-cta font-bold text-sm">{s.price}</p>
            </div>
          ))}
        </div>
      </div>
